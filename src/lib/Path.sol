...
library Path {
    ...
    /// @dev The offset of a sing token address + tick spacing
    uint256 private constant NEXT_OFFSET = ADDR_SIZE + TICK_SPACING_SIZE;
    /// @dev The offset of an encoded pool key (tokenIn + tickSpacing + tokenOut)
    uint256 private constant POP_OFFSET = NEXT_OFFSET + ADDR_SIZE;
    /// @dev The minimum length of a path that contains 2 or more pools
    uint256 private constant MULTIPLE_POOLS_MIN_LENGTH = POP_OFFSET + ADDR_SIZE;
